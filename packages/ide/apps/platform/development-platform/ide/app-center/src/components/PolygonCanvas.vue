<template>
  <div class="polygon-canvas-wrapper">
    <div class="canvas-toolbar" v-if="editMode === 'scene' || editMode === 'area'">
    ...
    <div class="canvas-toolbar" v-else-if="editMode === 'point-pick'">
      <el-button size="small" @click="clearPickedPoint" :disabled="!pickedPointInternal">清空选点</el-button>
      <span class="toolbar-tip">点击场景空间内任意位置设置设备点位</span>
    </div>
    <canvas
      ref="canvasRef"
      class="polygon-canvas"
      :class="{ 'is-readonly': editMode === 'view' }"
...
import type { AreaPolygonInfo, PolygonPoint } from '../types/scene'
...
const DEVICE_POINT_COLOR = '#f56c6c'

interface Props {
  scenePolygon: PolygonPoint[] | null
  areas: AreaPolygonInfo[]
  editMode: 'scene' | 'area' | 'view' | 'point-pick'
  editingAreaId?: string
  pickedPoint?: PolygonPoint | null
  devicePoints?: Array<{ id: string; name?: string; x: number; y: number }>
}
...
const emit = defineEmits<{
  ...
  updatePickedPoint: [point: PolygonPoint | null]
}>()

const canvasRef = ref<HTMLCanvasElement>()
const editingPoints = ref<PolygonPoint[]>([])
const pickedPointInternal = ref<PolygonPoint | null>(null)
...
watch(
  ...
  () => {
    ...
    pickedPointInternal.value = props.pickedPoint ? { ...props.pickedPoint } : null
    syncEditingPoints()
    render()
  }
)
...
watch(
  () => props.pickedPoint,
  (val) => {
    pickedPointInternal.value = val ? { ...val } : null
    render()
  },
  { deep: true }
)
...
function onMouseDown(e: MouseEvent) {
  if (props.editMode === 'view' || props.editMode === 'point-pick') return
...
function onMouseUp(e: MouseEvent) {
  if (props.editMode === 'point-pick') {
    const pt = screenToLogic(e.clientX, e.clientY)
    const target = { x: clamp(pt.x), y: clamp(pt.y) }
    if (!props.scenePolygon || props.scenePolygon.length < 3) {
      ElMessage.warning('请先在场景空间中定义场景多边形')
      return
    }
    if (!isPointInPolygon(target, props.scenePolygon)) {
      warnOutside()
      return
    }
    pickedPointInternal.value = target
    emit('updatePickedPoint', { ...target })
    render()
    return
  }
...
function clearPickedPoint() {
  pickedPointInternal.value = null
  emit('updatePickedPoint', null)
  render()
}
...
function warnOutside() {
  ...
  outsideWarnedAt = now
  if ((props.editMode === 'area' || props.editMode === 'point-pick') && (!props.scenePolygon || props.scenePolygon.length < 3)) {
    ElMessage.warning('请先定义场景空间，再继续操作')
  } else {
...
function drawDevicePoints(ctx: CanvasRenderingContext2D) {
  for (const p of props.devicePoints) {
    if (!Number.isFinite(p.x) || !Number.isFinite(p.y)) continue
    drawCrosshair(ctx, p.x, p.y, '#909399', 6)
    if (p.name) {
...
function drawPickedPoint(ctx: CanvasRenderingContext2D) {
  const pt = pickedPointInternal.value
  if (!pt || !Number.isFinite(pt.x) || !Number.isFinite(pt.y)) return
  drawCrosshair(ctx, pt.x, pt.y, DEVICE_POINT_COLOR, POINT_PICK_RADIUS)
  ctx.font = '12px sans-serif'
  ctx.fillStyle = DEVICE_POINT_COLOR
  ctx.textAlign = 'center'
  ctx.textBaseline = 'top'
  ctx.fillText(`(${pt.x}, ${pt.y})`, pt.x, pt.y + POINT_PICK_RADIUS + 4)
}
