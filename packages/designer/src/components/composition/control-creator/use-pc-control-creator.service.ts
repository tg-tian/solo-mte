import { getSchemaByType } from "@farris/ui-vue";
import { omit } from "lodash-es";
import { FormBindingType, FormSchemaEntityField, FormSchemaEntityFieldTypeName, UseControlCreator, UseSchemaService } from "../../types";
import { FormMetadataConverter } from "../form-metadata-converter";

export function usePCControlCreator(schemaService: UseSchemaService): UseControlCreator {
    /**
     * 配置输入控件属性
     ...
     * @returns 
     */
    function setFormFieldProperty(field: FormSchemaEntityField, editorType?: string, controlClass = 'col-12 col-md-6 col-xl-3 col-el-2', position = 'Card'): any {
        ...
        formGroupMetadata.editor = formEditor?.type ? formEditor : { type: 'input-group' };
        if (field.require) {
            formEditor.required = field.require;
        }

        // 只读属性：若字段本身为只读，则取字段属性；若非只读，则设置为状态机
        formEditor.readonly = field.readonly ? true : "!viewModel.stateMachine['editable']";
        ...
        // 如果是在卡片，并且是多行文本
        if (position === 'Card' && formEditor.type === 'textarea') {
            formEditor.resizable = true;
        }

        // 数字类型
        if (fieldTypeInSchema === 'NumericType') {
            if (formEditor.type === 'check-box' || formEditor.type === 'switch') {
                formEditor.trueValue = 1;
                formEditor.falseValue = 0;
            } else {
                formEditor.precision = field.type.precision;
                formEditor.nullable = true;
                if (position === 'Grid') {
                    formEditor.textAlign = 'right';
                }                
            }
        }
        // 数字、字符串、备注 ：设置最大长度
        if (['NumericType', 'StringType', 'TextType'].includes(fieldTypeInSchema)) {
            ...
            if (fieldTypeInSchema === 'StringType' && (formEditor.type === 'check-box' || formEditor.type === 'switch')) {
                formEditor.trueValue = 'true';
                formEditor.falseValue = 'false';
            }
        }
        // 日期类型
        ...
        // 帮助类型
        if (formEditor.type === 'lookup' && field.editor?.$type === 'lookup') {
            const editorOptions = omit(field.editor, '$type');
            Object.assign(formEditor, editorOptions);
        }

        schemaService.addRtcFieldsToSchemaEntity(field);
    ...
    function setGridFieldProperty(gridType: string, field: FormSchemaEntityField, metadata: any, needInlineEditor = false): any {
        ...
        if (needInlineEditor) {
            ...
            const fieldEditor = setFormFieldProperty(field, realEditor, '', 'Grid');
     ...
     * @param controlClass 输入控件样式
     */
    function createFormGroupWithoutField(editorType = 'input-group', controlClass = 'col-12 col-md-6 col-xl-3 col-el-2') {
        const formGroupMetadata = getSchemaByType('form-group') || {};
        ...
        // 因为设计器需要创建的控件属性与控件默认属性不一致，所以这里单独赋值
        switch (formEditor?.type) {
            case 'textarea': {
                // 多行文本
                formEditor.resizable = true;
                break;
            ...
            case 'radio-group': case 'check-group': case 'combo-list': {
                // 枚举类控件
                formEditor.data = [];
                formEditor.idField = 'value';
                formEditor.valueField = 'value';
    ...
    return {
        ...
        mapControlType2GridFieldType,
        setGridFieldFormatter
    };
}
