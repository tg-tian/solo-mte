...
import { expressInputParamsProps } from './express-input-params.props.ts';
import { useBem, uuid, ParamValidateUtils } from '@farris/flow-devkit';
import type { Parameter, TypeRefer, ValueExpress, ParamValidateOptions } from '@farris/flow-devkit';
import { BasicTypeRefer } from '@farris/flow-devkit';
import { TypeSelector, ValueExpressionInput } from '@farris/flow-devkit';

import './express-input-params.scss';


export default defineComponent({
  name: 'ExpressInputParams',
  props: expressInputParamsProps,
  emits: ['update:modelValue'],
  setup(props, context) {
    const { bem } = useBem('ExpressInputParams');
        ...
        <div class={bem('param')} key={param.id}>
          ...
          <div class={bem('input-item')} style="flex: 1; min-width: 100px;">
            <TypeSelector
              ...
              readonly={true}
              /*readonly={isParamReadonly}*/
              onUpdate:modelValue={(newType:any) => onUpdateParamType(param, newType)}
            ></TypeSelector>
          </div>
          <div class={bem('input-item')} style="flex: 3">
            <ValueExpressionInput
              ...
              onUpdate:modelValue={(newValue: ValueExpress, _: ValueExpress, { type }: { type?: TypeRefer }) => onUpdateParamValue(param, newValue, type)}
            ></ValueExpressionInput>
