import { type CompareOperatorType, CompareOperator } from "@farris/flow-devkit/types";

export function useCompareOperator() {
  ...
  function renderGreaterThan() {
    return (
      <svg
        ...
        fill="currentColor"
        xmlns="http://www.w3.org/2000/svg"
        aria-describedby="h20rd9x"
        tabindex="0"
        data-popupid="h20rd9x"
      >
  ...
  function renderGreaterThanEqual() {
    return (
      <svg
        width="1em"
        height="1em"
  ...
  function renderCompareOperator(operator?: CompareOperatorType) {
    if (!operator || !CompareOperator[operator]) {
      return;
    }
    switch (operator) {
      ...
      case CompareOperator.notEmpty:
        return renderNotEqual();
      case CompareOperator.greaterThan:
      case CompareOperator.lengthGreaterThan:
        return renderGreaterThan();
      case CompareOperator.greaterThanEqual:
      case CompareOperator.lengthGreaterThanEqual:
        return renderGreaterThanEqual();
      case CompareOperator.lessThan:
      case CompareOperator.lengthLessThan:
        return renderLessThan();
      case CompareOperator.lessThanEqual:
      case CompareOperator.lengthLessThanEqual:
        return renderLessThanEqual();
      case CompareOperator.contain:
        return renderContain();
      case CompareOperator.notContain:
        return renderNotContain();
    }
  }

  return {
    renderCompareOperator,
  };
}
