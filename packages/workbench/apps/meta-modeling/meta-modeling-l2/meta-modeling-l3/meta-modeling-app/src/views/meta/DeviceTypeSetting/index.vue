...
// 获取类型（device 或 component）（优先使用 props，否则使用路由参数）
...
// 监听路由或 props 变化，加载对应的设备类型数据
watch([() => route.query.deviceTypeId, () => route.query.mode, () => props.deviceTypeId, () => props.mode], async ([routeId, routeMode, propsId, propsMode]) => {
...
// 返回列表（如果是弹窗模式，发出 cancelled 事件；否则路由跳转）
const navigateBack = () => {
  if (props.mode !== undefined) {
    // 弹窗模式：发出取消事件
    emit('cancelled')
