...
Provide safe, irreversible deletion of app domains, modules, and apps in the app-center page, with confirmation dialogs and pre-deletion child-node checks.
...
#### Scenario: App domain with children shows warning notify

- **WHEN** user clicks the delete icon on an app domain that has one or more modules
- **THEN** the system displays a warning notification "应用域「{name}」下存在模块，请先删除所有模块后再删除应用域。"

#### Scenario: Delete app domain confirmation dialog

- **WHEN** user clicks the delete icon on an empty app domain
- **THEN** a modal dialog is displayed with a warning icon inline with title, message "您即将删除应用域「{name}」。这是永久性操作且不可撤销。", a cancel button, and a red confirm button labeled "确认删除"
...
#### Scenario: Module with children shows warning notify

- **WHEN** user clicks the delete icon on a module that has one or more apps
- **THEN** the system displays a warning notification "模块「{name}」下存在应用，请先删除所有应用后再删除模块。"

#### Scenario: Delete module confirmation dialog

- **WHEN** user clicks the delete icon on an empty module
- **THEN** a modal dialog is displayed with a warning icon inline with title, message "您即将删除模块「{name}」。这是永久性操作且不可撤销。", a cancel button, and a red confirm button labeled "确认删除"
...
- **THEN** a modal dialog is displayed with a warning icon inline with title, message "您即将删除应用「{name}」。这是永久性操作，所有工程文件都将被删除，不可撤销。", a text input labeled "请输入应用名称以确认", a cancel button, and a red confirm button labeled "确认删除"

#### Scenario: Delete app confirm validates name match

- **WHEN** user clicks "确认删除" in the app delete confirmation dialog
- **THEN** if the text input value does not match the app name exactly, a warning notification "请输入正确的应用名称" is shown and the dialog remains open
...
- **THEN** a delete icon (`f-icon-delete`) appears to the left of the expand/collapse chevron

#### Scenario: Delete icon appears on module hover

- **WHEN** user hovers the mouse over a module item in the module list
- **THEN** a delete icon (`f-icon-delete`) appears within the module item to the right
